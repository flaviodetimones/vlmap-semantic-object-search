...
\paragraph{Construcción del VLMap desde ROS/Gazebo.} Como paso hacia la
migración robótica, se amplió el flujo de construcción de mapas para que no
dependa únicamente de escenas HSSD ya preparadas. El contenedor ROS ejecuta el
HSR en Gazebo, captura RGB-D, profundidad y pose de base, y vuelca esos datos en
el mismo formato que consume el constructor original de VLMaps. Después,
\texttt{tfg-sim} aplica el mismo proceso LSeg+CLIP que en las escenas HSSD y
produce un fichero \texttt{vlmaps.h5df} compatible con el resto del sistema.

Durante esta integración se corrigieron varios fallos de ingeniería que
impedían cerrar el ciclo completo: la captura tolera errores temporales de TF y
puede caer a \texttt{/odom} cuando el árbol \texttt{map} todavía no está unido,
el menú carga el directorio correcto del run, el entorno expone el
\texttt{PYTHONPATH} necesario para el paquete local, la configuración Hydra
sobrescribe claves existentes con \texttt{++} y el cargador inyecta los
parámetros \texttt{grid\_size} y \texttt{cell\_size} antes de inicializar el
mapa. Con ello, la captura en Gazebo y la construcción en el contenedor
semántico quedan unidas por el volumen compartido \texttt{/shared}.

\paragraph{Validación multiclase del mapa capturado.} El primer análisis del
mapa usaba el score crudo de una única categoría, pero esa medida no era
adecuada: las puntuaciones de CLIP para categorías aisladas pueden ser casi
colineales y producir mapas muy correlacionados aunque la escena esté bien
representada. Por ello se implementó una validación sin interfaz gráfica que reproduce el
criterio nativo de VLMaps. Primero se inicializa el vocabulario completo de
Matterport3D junto con las categorías pedidas; después cada vóxel se asigna sólo
a la categoría que gana frente a todas las demás y frente a la clase de fondo
\texttt{other}. Este criterio permite medir regiones dominantes y no simples
activaciones sueltas.
...
\bottomrule
\end{tabular}
\caption[Validación del índice semántico multiclase.]{Validación del índice
semántico multiclase. En el mapa capturado desde ROS/Gazebo las categorías de
habitaciones distintas quedan prácticamente disjuntas; en HSSD el mayor solape
aparece entre mesa y silla, una relación esperable por proximidad física.}
\label{tab:vlmap_index_multiclase}
\end{table}

La captura \texttt{small\_house/run\_20260607\_174653}, con 474 frames, separa
la zona de cama, la zona de cocina, el área común con sofá y sillas, y deja
\texttt{toilet} vacío al no estar presente de forma estable. La comparación con
HSSD muestra que el mapa capturado desde ROS/Gazebo es menos denso, pero sí
discrimina las categorías principales. Si se quisiera mejorar la cobertura, la
acción concreta sería repetir la captura con un recorrido más denso alrededor de
muebles y esquinas, no cambiar el formato del mapa.

\begin{figure}[H]
\centering
\includegraphics[width=0.48\linewidth]{Pictures/vlmap_index_small_house.png}
\hfill
\includegraphics[width=0.48\linewidth]{Pictures/vlmap_index_hssd_102344280_0.png}
\caption[Validación visual del índice VLMaps multiclase.]{Validación visual del
índice VLMaps multiclase. Izquierda: captura ROS/Gazebo de \texttt{small\_house}.
Derecha: mapa HSSD \texttt{102344280\_0} usado como referencia. Cada color
representa la categoría dominante por celda.}
\label{fig:vlmap_index_multiclase}
\end{figure}
