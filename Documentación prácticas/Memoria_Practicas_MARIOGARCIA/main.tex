...
\paragraph{Mejora robusta en cuatro capas.} A partir de las pruebas con objetos pequeños se añadió una mejora específica para evitar dos fallos observados: perder una detección intermitente al acercarse y quedarse bloqueado cuando la línea recta hacia el objeto queda tapada por un mueble. La solución no depende de un único umbral instantáneo de YOLOE, sino de cuatro capas encadenadas. Primero, la exploración se pondera con la evidencia semántica de VLMaps para visitar antes los puntos cercanos a soportes plausibles. Segundo, cada detección visual se guarda como hipótesis espacial con pose, caja, confianza y celda proyectada en el mapa. Tercero, la confirmación usa evidencia temporal: varias detecciones próximas, estables o con caja creciente pueden confirmar mejor que un solo frame aislado. Cuarto, si la aproximación frontal falla, el planificador busca varias vistas navegables alternativas alrededor de la hipótesis visual y vuelve a verificar desde cada una.

\begin{figure}[H]
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[node distance=0.8cm and 0.95cm]
  \node[bloque, minimum width=3.4cm, minimum height=1.05cm] (sem) {1. Prior semántico\\\footnotesize VLMaps + habitación + soportes};
  \node[bloquen, minimum width=3.4cm, minimum height=1.05cm, right=of sem] (mem) {2. Memoria visual\\\footnotesize pose, caja, confianza, celda};
  \node[bloquev, minimum width=3.4cm, minimum height=1.05cm, right=of mem] (temp) {3. Evidencia temporal\\\footnotesize estabilidad y crecimiento};
  \node[bloque, minimum width=3.4cm, minimum height=1.05cm, right=of temp] (multi) {4. Aproximación multivista\\\footnotesize rutas navegables alternativas};

  \node[draw=none, fill=none, below=0.55cm of sem, align=center, font=\sffamily\footnotesize, text width=3.8cm] (semTxt) {Ordena puntos de exploración alrededor de muebles activados por el mapa semántico.};
  \node[draw=none, fill=none, below=0.55cm of mem, align=center, font=\sffamily\footnotesize, text width=3.8cm] (memTxt) {Permite recuperar el objetivo aunque YOLOE parpadee durante unos frames.};
  \node[draw=none, fill=none, below=0.55cm of temp, align=center, font=\sffamily\footnotesize, text width=3.8cm] (tempTxt) {Evita decidir por un único frame y acumula confianza visual cercana.};
  \node[draw=none, fill=none, below=0.55cm of multi, align=center, font=\sffamily\footnotesize, text width=3.8cm] (multiTxt) {Si un obstáculo tapa el frente, prueba otros standoff seguros alrededor de la hipótesis.};

  \draw[flecha] (sem) -- (mem);
  \draw[flecha] (mem) -- (temp);
  \draw[flecha] (temp) -- (multi);
  \draw[flecha] (sem) -- (semTxt);
  \draw[flecha] (mem) -- (memTxt);
  \draw[flecha] (temp) -- (tempTxt);
  \draw[flecha] (multi) -- (multiTxt);

  \node[grupo, fit=(sem) (mem) (temp) (multi) (semTxt) (memTxt) (tempTxt) (multiTxt),
        label={[anchor=south, font=\sffamily\bfseries\small, text=figAzulBorde]north:sistema robusto de búsqueda y confirmación}] {};
\end{tikzpicture}%
}
\caption[Mejora robusta en cuatro capas.]{Mejora robusta en cuatro capas para objetos pequeños: prior semántico para elegir vistas, memoria visual para no perder hipótesis, evidencia temporal para confirmar con estabilidad y aproximación multivista para rodear obstáculos.}
\label{fig:mejora_cuatro_capas}
\end{figure}
