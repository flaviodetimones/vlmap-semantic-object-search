...
\usetikzlibrary{arrows.meta,positioning,fit,shapes.multipart,shapes.geometric}
...
\newcommand{\optionalplot}[2][]{%
  \IfFileExists{#2}{%
    \includegraphics[#1]{#2}%
  }{%
    \fbox{\parbox[c][3.0cm][c]{0.46\linewidth}{\centering\footnotesize Figura no disponible}}%
  }%
}
...
\begin{figure}[htbp]
\centering
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/heatmap_phase/20260424_153612/plots/bar_hitat_1.png}\hfill
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/heatmap_phase/20260424_153612/plots/bar_mass_in_expected_ratio.png}\\[6pt]
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/heatmap_phase/20260424_153612/plots/bar_iou_topmass50.png}\hfill
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/heatmap_phase/20260424_153612/plots/bar_wrong_room_mass_ratio.png}
...
\begin{figure}[htbp]
\centering
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/orchestrator_phase/20260424_154155/plots/sr.png}\hfill
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/orchestrator_phase/20260424_154155/plots/cfr.png}
...
\begin{figure}[htbp]
\centering
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/orchestrator_phase/20260424_154155/plots/wrong_visits.png}\hfill
\optionalplot[width=0.49\linewidth]{vlmap-semantic-object-search-tfg/results/orchestrator_phase/20260424_154155/plots/mean_pose_updates.png}
...
\begin{enumerate}
  ...
  \item \textbf{Acercamiento antes de confirmar}: una detección visual ya no
        se acepta como éxito final por confianza alta desde lejos. Cualquier
        disparo de YOLOE pasa primero por una fase común de acercamiento:
        orientar la cámara, intentar un acercamiento planificado al mueble
        soporte cuando existe, avanzar sólo si la siguiente celda es navegable
        y permanece dentro de la habitación fijada, y repetir la confirmación
        con umbral alto desde cerca.
  \item \textbf{Circuito fijo por habitación con disparo en ruta}: la opción
        \texttt{x} mantiene un número fijo de puntos por habitación; por
        defecto, 8. La generación de puntos ya no usa una rejilla rectangular
        sobre la caja envolvente de la habitación: ahora agrupa directamente las
        celdas navegables LabelMe reales con una selección tipo
        \emph{k-medoids}, prueba tres niveles de candidatos
        (margen preferido, margen relajado y cualquier celda navegable), aplica
        una separación mínima configurable
        (\texttt{VLMAPS\_EXPLORE\_POINT\_MIN\_SEP\_M}, 0,90\,m por defecto) y
        ordena el resultado por vecino más cercano desde la pose actual. Esto
        evita que una región no convexa concentre todos los puntos en el centro
        o deje sin cubrir esquinas útiles. La opción \texttt{x} también activa
        \texttt{VLMAPS\_EXPLORE\_DEBUG=1}, guardando una imagen por exploración
        en \texttt{debug/zone\_explore\_<room>\_<target>.png} con máscara,
        candidatos y circuito elegido. Mientras el robot recorre ese circuito,
        YOLOE sigue vigilando con umbral bajo
        0.40: si aparece una detección, la ruta se pausa, se abandona
        temporalmente el punto del circuito y el robot proyecta una meta
        navegable en la dirección de la cámara que acaba de ver el objeto. La
        confirmación se hace con una sesión YOLOE separada a 0.80, de modo que
        el umbral bajo de aviso y el umbral alto de confirmación no se pisan.
        Si no hay disparo en ruta, al llegar a cada punto hace un barrido
        lateral $\pm 30^\circ$ y pasa al siguiente.
\end{enumerate}
...
\subsection*{11 de mayo de 2026 --- Ajuste de navegación estrecha y acercamiento visual}

\begin{enumerate}
  \item \textbf{Inflación de planificación reducida}: el mapa usado para el
        grafo de planificación pasa de 3 a 2 iteraciones de dilatación por
        defecto (\(\sim\)10\,cm por lado con celdas de 5\,cm). Esto permite
        entrar en pasos más estrechos sin desactivar el escudo de ejecución,
        que sigue usando el mapa bruto de navegación. El valor queda
        parametrizado con \texttt{VLMAPS\_PLAN\_DILATION\_ITERS}; para volver
        al comportamiento anterior basta lanzar con valor 3.
  \item \textbf{Corrección del giro hacia objetivo}: la rutina
        \texttt{face\_toward\_pos()} se alinea con la convención angular real
        observada en Habitat: \(0^\circ\) mira hacia el norte del mapa y, en
        este wrapper, \texttt{turn\_left} incrementa el ángulo almacenado.
        Antes mezclaba otra convención y podía orientar la cámara hacia el lado
        opuesto del objeto detectado.
  \item \textbf{Acercamiento más permisivo y trazable}: los puntos de
        acercamiento a una detección visual usan ahora una holgura mínima de
        2 celdas por defecto, configurable con
        \texttt{VLMAPS\_APPROACH\_MIN\_CLEARANCE\_CELLS}. Si el mapa inflado no
        ofrece ningún punto viable, se intenta una segunda selección sobre el
        mapa bruto navegable y se conserva el filtro de no salir de la región
        LabelMe fijada.
  \item \textbf{Proyección visual menos conservadora}: cuando YOLOE dispara
        con umbral bajo durante la exploración, la celda objetivo proyectada en
        dirección de la cámara se calcula sobre el mapa bruto navegable. Esto
        evita cortar la proyección demasiado pronto cerca de muebles, que era
        una de las causas de acercamientos que apenas se movían.
  \item \textbf{Recuperación ante fallo de sesión YOLOE}: si durante el
        centrado, el acercamiento o la confirmación final aparece un
        \texttt{Broken pipe}, se reabre la sesión de YOLOE y se reintenta una
        vez antes de descartar el acercamiento. El objetivo sigue siendo el
        mismo: detección lejana a 0,40 sólo dispara acercamiento; éxito final
        únicamente con confirmación cercana a 0,80.
\end{enumerate}

\subsection*{12 de mayo de 2026 --- Servo visual antes de planificación en detecciones YOLOE}

\begin{enumerate}
  \item \textbf{Acercamiento guiado por la detección}: cuando YOLOE detecta un
        objeto con umbral bajo durante la exploración, el robot ya no gira
        primero hacia una celda proyectada ni navega directamente a un
        \textit{standoff} lateral. Primero centra la caja de YOLOE y avanza por
        pasos seguros mientras respeta obstáculos y la región LabelMe activa.
        La planificación A* queda como respaldo si el acercamiento frontal no
        logra avanzar.
  \item \textbf{Menos cambios de objetivo durante el acercamiento}: el flujo
        anterior podía perder la botella detectada, girar hacia otra zona y
        enganchar otra botella parecida. El nuevo orden mantiene la referencia
        visual original como prioridad y evita preplanificar hacia muebles o
        celdas estimadas antes de intentar el acercamiento visual.
  \item \textbf{Confirmación estricta}: la detección lejana a
        \texttt{0.40} sigue siendo sólo un disparador. La búsqueda únicamente
        se marca como exitosa tras confirmación cercana con el umbral alto
        configurado, por defecto \texttt{0.80}.
  \item \textbf{Resumen de consola legible}: el bloque JSON completo
        \texttt{[eval-summary]} deja de imprimirse en modo interactivo por
        defecto. En su lugar aparece un resumen corto con objetivo, habitación,
        puntos explorados, detecciones de bajo umbral, acercamientos y falsas
        alarmas. Para evaluaciones automáticas se conserva el JSON activando
        \texttt{VLMAPS\_EVAL\_SUMMARY\_JSON=1}; en modo headless sigue activo
        por defecto.
\end{enumerate}

\subsection*{12 de mayo de 2026 --- Puntos de exploración adaptativos por región}

\begin{enumerate}
  \item \textbf{Modo automático por defecto}: el flujo \texttt{x} ya no fuerza
        siempre 8 puntos por habitación. Si se deja el valor por defecto
        \texttt{auto}, el sistema calcula el número de puntos según el área
        navegable LabelMe de la región: una habitación de tamaño de referencia
        recibe 8 puntos, una de doble tamaño tiende a 16 y una de tres cuartos
        tiende a 6.
  \item \textbf{Límites prácticos}: para evitar rutas demasiado largas o zonas
        pequeñas mal cubiertas, el modo automático queda acotado por defecto
        entre 4 y 20 puntos, configurable con
        \texttt{VLMAPS\_EXPLORE\_MIN\_POINTS} y
        \texttt{VLMAPS\_EXPLORE\_MAX\_POINTS\_CAP}. El número base por región
        de referencia se controla con
        \texttt{VLMAPS\_EXPLORE\_POINTS\_PER\_REFERENCE}.
  \item \textbf{Corrección por forma}: además del área, se aplica un pequeño
        factor de forma. Las regiones alargadas o con baja ocupación dentro de
        su caja envolvente reciben algún punto adicional porque suelen tener
        brazos, esquinas o pasillos que un muestreo puramente proporcional
        puede ignorar.
  \item \textbf{Reproducibilidad}: si se escribe un número en el menú
        \texttt{x}, se desactiva el modo automático y se mantiene exactamente
        ese número de puntos, como antes. Esto permite repetir experimentos con
        una configuración fija cuando interese comparar resultados.
\end{enumerate}

\subsection*{12 de mayo de 2026 --- Ajuste de puntos y acercamiento a detecciones}

\begin{enumerate}
  \item \textbf{Menos puntos por defecto}: el modo automático deja de usar la
        mediana de habitaciones como unidad principal, porque en escenas con
        habitaciones pequeñas hacía que una cocina de tamaño medio escalase a
        12 puntos. La referencia pasa a ser física, 8\,m\(^2\), con 8 puntos
        base, mínimo 4 y máximo 16. En la práctica, una cocina de unos
        7--8\,m\(^2\) vuelve a generar alrededor de 8 puntos.
  \item \textbf{Centrado visual con memoria}: durante el centrado YOLOE se
        guarda la última caja detectada. Si el último giro hace perder el
        objeto, se deshace ese giro una vez y el acercamiento conserva la caja
        anterior como pista visual. Esto evita que el robot corte el
        acercamiento tras un único giro demasiado agresivo.
  \item \textbf{Respaldo planificado más cercano}: si el acercamiento visual no
        alcanza la confirmación de 0,80, el respaldo A* ya no prioriza el punto
        de mayor holgura alrededor de la detección. Ahora prioriza el punto
        navegable viable más cercano a la celda proyectada del objeto, y al
        llegar se orienta explícitamente hacia esa celda antes de volver a
        confirmar.
  \item \textbf{Persistencia tras avance parcial}: aunque el servo visual haya
        avanzado algo, si no confirma desde cerca se intenta igualmente el
        respaldo al punto cercano al objeto. La detección de 0,40--0,79 sigue
        siendo sólo un disparador; el éxito continúa requiriendo confirmación
        cercana con el umbral alto.
\end{enumerate}

\subsection*{27 de mayo de 2026 --- Robustez de exploración, detecciones persistentes y filtrado navegable}

Tras las pruebas con \texttt{mug}, \texttt{basketball}, \texttt{teddy bear},
\texttt{bottle} y \texttt{laptop} en las escenas HSSD, se detectaron tres
fallos prácticos: puntos de exploración que visualmente caían en la habitación
pero no eran alcanzables por el robot, detecciones de baja probabilidad que
interrumpían demasiado pronto el circuito de cobertura, y una persistencia
visual que no debía dibujar cajas propias sino conservar exactamente la
anotación producida por YOLOE.

\begin{enumerate}
  \item \textbf{Persistencia visual basada exclusivamente en YOLOE}: cuando
        YOLOE devuelve una detección positiva se guarda el frame anotado por el
        propio detector (\texttt{ann\_rgb}). Si en los siguientes frames la
        detección parpadea brevemente, la interfaz reutiliza ese frame anotado.
        No se dibujan cajas adicionales desde la capa de navegación. La marca
        persistente, por tanto, es la misma caja azul/verde generada por YOLOE,
        con su etiqueta y confianza originales.
  \item \textbf{Caducidad espacial de la marca}: la persistencia anterior no
        equivale a mostrar evidencia obsoleta. Si el robot gira demasiado, se
        desplaza a otra pose o la detección envejece, la vista vuelve a la
        cámara RGB en vivo. Esto evita el caso observado en el que una caja
        permanecía en pantalla aunque el objeto ya no estuviese visible.
  \item \textbf{Filtrado de puntos realmente navegables}: antes de aceptar un
        punto de exploración se valida la ruta completa con el mismo mapa usado
        para planificar. El punto se descarta si cae en obstáculo, si su
        holgura es insuficiente, si A* no llega al objetivo, si la ruta cruza
        obstáculos o si sale indebidamente de la máscara de habitación. En los
        logs aparece como \texttt{Puntos descartados por no alcanzables}.
  \item \textbf{Sobremuestreo y selección posterior}: la generación ya no pide
        exactamente $N$ puntos y los acepta sin más. Primero genera más
        candidatos de los necesarios, después filtra por alcanzabilidad real y
        finalmente conserva los mejores $N$. Esto permite mantener cobertura
        aunque varios puntos aparentemente buenos resulten imposibles para la
        huella del robot.
  \item \textbf{Rutas que empiezan fuera y reentran}: si el robot viene de otra
        habitación, una ruta válida puede empezar fuera de la máscara objetivo
        y entrar inmediatamente en ella. El validador permite ese caso, pero
        rechaza trayectorias que atraviesan o abandonan la habitación una vez
        dentro.
  \item \textbf{Continuación del circuito tras detección no confirmada}: una
        detección baja o incluso una alta probabilidad no confirmada desde
        cerca ya no aborta la exploración de la habitación. El sistema intenta
        confirmar desde el punto actual; si no lo consigue, continúa hacia el
        resto de puntos para buscar otro ángulo. Esto corrige casos como
        \texttt{mug} y \texttt{basketball}, donde una vista lateral débil no
        bastaba para confirmar.
  \item \textbf{Mejora de soportes semánticos VLMaps}: se amplían los muebles
        usados para focalizar la exploración. Además de \texttt{table},
        \texttt{counter}, \texttt{shelf} y \texttt{cabinet}, se incluyen
        \texttt{bed}, \texttt{sofa} y \texttt{chair}. Esto mejora objetos como
        \texttt{basketball} y \texttt{teddy bear}, que no siempre aparecen
        cerca de superficies de cocina.
  \item \textbf{Alias y priors nuevos}: se añaden alias en inglés y español
        para \texttt{teddy bear} (\texttt{peluche}, \texttt{oso de peluche},
        \texttt{stuffed bear}) y se ajustan los priors de habitación para
        \texttt{ball}/\texttt{basketball} y \texttt{teddy bear}. La intención
        es que el room-first no busque únicamente donde el objeto es visible
        para YOLOE, sino donde domésticamente tiene sentido.
\end{enumerate}
...
\begin{tikzpicture}[
  ...
  decision/.style={draw, diamond, aspect=2.2, align=center, fill=orange!10},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
  \node[box] (query) {Query\\objeto + sala opcional};
  \node[box, right=of query] (priors) {Priors\\habitación + soportes};
  \node[box, right=of priors] (vlmaps) {VLMaps\\heatmap de muebles};
  \node[box, below=of vlmaps] (points) {Candidatos\\sobremuestreados};
  \node[decision, left=of points] (valid) {Ruta\\usable?};
  \node[box, left=of valid] (nav) {Navegar\\punto válido};
  \node[box, below=of nav] (yoloe) {YOLOE activo\\durante ruta y barrido};
  \node[decision, right=of yoloe] (det) {Detección\\positiva?};
  \node[box, right=of det] (approach) {Centrar + acercar\\confirmación alta};
  \node[decision, below=of approach] (confirmed) {Confirmado?};
  \node[box, left=of confirmed] (continue) {Seguir circuito\\otro punto/ángulo};
  \node[box, right=of confirmed, fill=green!10] (found) {Objeto encontrado\\frame YOLOE persistente};

  \draw[arrow] (query) -- (priors);
  \draw[arrow] (priors) -- (vlmaps);
  \draw[arrow] (vlmaps) -- (points);
  \draw[arrow] (points) -- (valid);
  \draw[arrow] (valid) -- node[above]{sí} (nav);
  \draw[arrow] (valid.north) |- node[pos=0.25,right]{no} (points.west);
  \draw[arrow] (nav) -- (yoloe);
  \draw[arrow] (yoloe) -- (det);
  \draw[arrow] (det) -- node[above]{sí} (approach);
  \draw[arrow] (det.south) |- node[pos=0.25,right]{no} (continue.west);
  \draw[arrow] (approach) -- (confirmed);
  \draw[arrow] (confirmed) -- node[above]{sí} (found);
  \draw[arrow] (confirmed) -- node[above]{no} (continue);
  \draw[arrow] (continue.west) -| (points.south);
\end{tikzpicture}
\caption{Flujo robusto tras la corrección del 27 de mayo: VLMaps propone zonas
semánticas, el planificador descarta puntos no alcanzables, YOLOE verifica
durante navegación y sólo la anotación original del detector se mantiene de
forma temporal en pantalla.}
\label{fig:flujo-robusto-27-mayo}
\end{figure}

\subsection*{28 de mayo de 2026 --- Smoke real de doble contenedor (tfg-sim $\rightarrow$ tfg-ros)}

Cerrada la navegación base del Toyota HSR en Gazebo (Fase~1), se valida por
primera vez el \emph{contrato} completo entre los dos contenedores (Fase~2):
desde \texttt{tfg-sim} se pide una navegación a \texttt{tfg-ros} sin conocer
nada del robot salvo el contrato ROS, y vuelve un resultado con token,
éxito/fracaso y habitación. La regla arquitectónica se mantiene: la lógica de
decisión sigue en \texttt{tfg-sim} y ROS actúa sólo como \emph{backend} de
navegación y sensores.

\paragraph{Comandos de la prueba.} La prueba usa tres terminales. La primera
arranca el robot, su navegación y el puente \texttt{rosbridge} dentro de
\texttt{tfg-ros}:

\begin{verbatim}
# Terminal 1 (tfg-ros): Gazebo + move_base + RViz + rosbridge
roslaunch vlmap_bringup hsr_gazebo_move_base.launch \
    gui:=true use_rviz:=true use_rosbridge:=true
\end{verbatim}

La segunda observa el topic neutral de resultado dentro de \texttt{tfg-ros}:

\begin{verbatim}
# Terminal 2 (tfg-ros): observador del resultado
rostopic echo /vlmap/navigation_result
\end{verbatim}

La tercera envía el \texttt{SemanticGoal} desde \texttt{tfg-sim} a través de
\texttt{rosbridge}, usando la herramienta \texttt{send\_ros\_semantic\_goal.py}
y el \texttt{RosNavigationBackend}:

\begin{verbatim}
# Terminal 3 (tfg-sim): enviar el goal por rosbridge
python tools/send_ros_semantic_goal.py \
    --host tfg-ros --x 0.50 --y 0.50 --yaw 0.50 \
    --room office --object laptop --timeout 30
\end{verbatim}

\paragraph{Resultado observado.} El robot gira y avanza hasta la pose pedida
sin movimientos erráticos (gracias al \emph{cmd\_vel smoother} y al giro
limitado a \texttt{max\_vel\_theta}~$=0{,}22$). El recorrido del mensaje fue
el esperado:
\begin{enumerate}[noitemsep]
  \item \texttt{tfg-sim} publica el goal y \texttt{tfg-ros} recibe el
        \texttt{SemanticGoal} con su \texttt{navigation\_token}.
  \item se reexpide a \texttt{/move\_base\_simple/goal} como
        \texttt{PoseStamped} con \emph{yaw} real.
  \item \texttt{move\_base} planifica, navega y emite
        \texttt{/move\_base/result} con \texttt{status}~$=3$ (\emph{SUCCEEDED}).
  \item el puente devuelve \texttt{/vlmap/navigation\_result} con el mismo
        token, \texttt{success}~$=\texttt{true}$ y \texttt{actual\_room}.
\end{enumerate}

El JSON de resultado confirma el cierre del contrato (token idéntico de ida y
vuelta, \texttt{result\_status\_code}~$=3$). En esta corrida
\texttt{room\_source}~$=$\,\texttt{requested\_echo}: la habitación devuelta es
todavía un eco de la pedida, porque el contexto semántico vivo aún no se
publicaba. Esa pieza es justo la siguiente (Fase~3): \texttt{tfg-sim} publica
\texttt{/vlmap/room\_context} (latched, JSON en \texttt{std\_msgs/String}) con
habitaciones y centroides, y \texttt{tfg-ros} resuelve \texttt{actual\_room}
desde la pose final real (\texttt{room\_source}~$=$\,\texttt{pose\_resolved}).

\begin{figure}[h]
\centering
\begin{tikzpicture}[
  node distance=0.7cm and 1.15cm,
  sim/.style={draw, rounded corners, align=center, minimum width=3.1cm,
              minimum height=0.9cm, fill=blue!8},
  ros/.style={draw, rounded corners, align=center, minimum width=3.1cm,
              minimum height=0.9cm, fill=green!8},
  bridge/.style={draw, align=center, minimum width=2.2cm, minimum height=0.8cm,
                 fill=orange!12},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
  % tfg-sim column
  \node[sim] (goal) {\texttt{send\_ros\_semantic\_goal.py}\\(query: office + laptop)};
  \node[sim, below=of goal] (backend) {\texttt{RosNavigationBackend}\\arma SemanticGoal + token};
  \node[sim, below=of backend] (wait) {Espera \texttt{navigation\_result}\\por token};

  % rosbridge
  \node[bridge, right=2.0cm of backend] (rb) {rosbridge\\WebSocket :9090};

  % tfg-ros column
  \node[ros, right=2.0cm of rb] (sg) {\texttt{/vlmap/semantic\_goal}};
  \node[ros, below=of sg] (mbsimple) {\texttt{/move\_base\_simple/goal}\\PoseStamped + yaw};
  \node[ros, below=of mbsimple] (mb) {\texttt{move\_base}\\plan + navegación HSR};
  \node[ros, below=of mb] (res) {\texttt{/move\_base/result}\\status = 3};
  \node[ros, below=of res] (navres) {\texttt{/vlmap/navigation\_result}\\token + success + room};

  \draw[arrow] (goal) -- (backend);
  \draw[arrow] (backend) -- (rb);
  \draw[arrow] (rb) -- (sg);
  \draw[arrow] (sg) -- (mbsimple);
  \draw[arrow] (mbsimple) -- (mb);
  \draw[arrow] (mb) -- (res);
  \draw[arrow] (res) -- (navres);
  \draw[arrow] (navres.west) -- ++(-0.6,0) |- (rb.south);
  \draw[arrow] (rb.west) -- ++(-0.5,0) |- (wait.east);

  \node[draw, dashed, fit=(goal)(backend)(wait), inner sep=0.35cm,
        label=above:{\textbf{tfg-sim} (decisión semántica)}] {};
  \node[draw, dashed, fit=(sg)(mbsimple)(mb)(res)(navres), inner sep=0.35cm,
        label=above:{\textbf{tfg-ros} (navegación / sensores)}] {};
\end{tikzpicture}
\caption{Flujo del smoke de doble contenedor (28 de mayo). \texttt{tfg-sim}
decide y publica un \texttt{SemanticGoal} por \texttt{rosbridge};
\texttt{tfg-ros} lo reexpide a \texttt{move\_base}, navega el HSR y devuelve
\texttt{navigation\_result} con el mismo token. La inteligencia no se porta a
ROS: éste actúa sólo como backend de navegación.}
\label{fig:smoke-doble-contenedor-28may}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{figs/demo_doble_contenedor_28may.png}
\caption{Evidencia de la prueba: RViz (arriba izq.) y Gazebo (abajo izq.) con
el Toyota HSR; a la derecha, \texttt{tfg-sim} enviando el goal
(\texttt{token=8002bfda\ldots}, \texttt{goal=(0{,}5,\,0{,}5,\,yaw=0{,}5)}) y
\texttt{tfg-ros} mostrando \texttt{/vlmap/navigation\_result} con
\texttt{success=true}, \texttt{result\_status\_code=3} y \texttt{actual\_room=office}.}
\label{fig:evidencia-smoke-28may}
\end{figure}

\subsection*{28 de mayo de 2026 (segunda iteración) --- Contexto semántico vivo (Fase 3) y giro más lento}

Cerrado el contrato de navegación (Fase~2), se implementa la \textbf{Fase~3}:
\texttt{tfg-sim} pasa a publicar el contexto semántico de habitaciones en vivo,
de modo que \texttt{tfg-ros} resuelve la habitación real desde la pose final
del robot en lugar de hacer eco de la pedida. Se respeta la regla
arquitectónica: el contexto viaja como \emph{datos} (no como geometría de
Gazebo), así que el mundo de simulación robótica no necesita cargar las
habitaciones.

\begin{enumerate}
  \item \textbf{Transporte elegido}: topic \emph{latched}
        \texttt{/vlmap/room\_context} con JSON en \texttt{std\_msgs/String}
        (mismo patrón que \texttt{SemanticGoal} y \texttt{navigation\_result}).
        Los mapas/heatmaps, si se envían, viajan como \emph{referencias} a
        \texttt{/shared/} (bind mount), nunca embebidos, para no saturar
        \texttt{rosbridge}.
  \item \textbf{Publicador (lado tfg-sim)}: nueva clase
        \texttt{RoomContextPublisher} en \texttt{tfg\_nav\_contracts}, simétrica
        a \texttt{RosNavigationBackend} (roslibpy sobre rosbridge, sin
        dependencia de \texttt{rospy}). \texttt{build\_payload()} arma
        \texttt{\{rooms, priors, maps\}} y \texttt{publish()} lo emite latched.
  \item \textbf{Resolución por pose (lado tfg-ros)}: se añade
        \texttt{DynamicRoomContext.room\_at\_pose(x, y, max\_distance)}, que
        devuelve la habitación cuyo centroide está más cerca de la pose final.
        El nodo \texttt{vlmap\_navigation\_result\_node} se suscribe a
        \texttt{/vlmap/room\_context} (latched) y a la pose del robot
        (\texttt{/amcl\_pose}); al cerrar un goal resuelve
        \texttt{actual\_room} desde la pose real.
  \item \textbf{Trazabilidad}: \texttt{build\_navigation\_result\_payload}
        acepta \texttt{resolved\_room}; cuando hay contexto vivo el resultado
        lleva \texttt{room\_source=pose\_resolved} y guarda
        \texttt{requested\_room}; sin contexto mantiene el fallback
        \texttt{room\_source=requested\_echo}.
  \item \textbf{Herramienta de disparo}: \texttt{tools/publish\_room\_context\_live.py},
        simétrica a \texttt{send\_ros\_semantic\_goal.py}. Modos: \texttt{--demo}
        (contexto de 3--4 salas con coordenadas en el frame \texttt{/map}),
        \texttt{--context-file} (JSON listo) y \texttt{--scene} (centroides de
        rejilla de \texttt{room\_map/regions.json}). Por defecto mantiene la
        conexión abierta para que el topic latched siga disponible a
        suscriptores tardíos; \texttt{--once} publica y sale.
  \item \textbf{Tests}: nuevo \texttt{tests/test\_room\_context\_live.py}
        (nearest-centroid, round-trip publicador$\leftrightarrow$parser,
        override de \texttt{actual\_room}). Total \textbf{27 pruebas en verde}.
\end{enumerate}

\paragraph{Smoke de Fase 3.} Con \texttt{tfg-ros} arrancado, desde
\texttt{tfg-sim}:

\begin{verbatim}
# 1) publicar contexto vivo (se queda latched)
python tools/publish_room_context_live.py --host tfg-ros --demo
# 2) mandar el goal a una sala conocida del contexto
python tools/send_ros_semantic_goal.py \
    --host tfg-ros --x 2.5 --y 0.0 --yaw 0.0 \
    --room office --object laptop --timeout 30
\end{verbatim}

Criterio de cierre: el resultado pasa de \texttt{room\_source="requested\_echo"}
a \texttt{"pose\_resolved"} y \texttt{actual\_room} se deriva de la pose final
real, eliminando la dependencia de centroides demo-estáticos.

\paragraph{Cierre verificado y ajuste de la fuente de pose.} En la primera
ejecución el resultado seguía marcando \texttt{requested\_echo} pese a publicar
el contexto. La causa: el \emph{launch} \texttt{hsr\_gazebo\_move\_base} no
incluye AMCL ni \texttt{map\_server} (sólo un TF estático
\texttt{map}$\rightarrow$\texttt{odom} identidad), de modo que
\texttt{/amcl\_pose} nunca se publica y la pose del robot quedaba a
\texttt{None}. Se corrige apuntando la fuente de pose por defecto a
\texttt{/odom} (\texttt{nav\_msgs/Odometry}): como \texttt{map}$\rightarrow$%
\texttt{odom} es identidad, las poses de \texttt{/odom} ya están en el frame
\texttt{map}. Un parámetro \texttt{\~{}robot\_pose\_type}
(\texttt{odom}\,$|$\,\texttt{amcl}\,$|$\,\texttt{pose\_stamped}) mantiene el
soporte para configuraciones con AMCL. Tras el ajuste, el smoke de doble
contenedor devuelve \texttt{room\_source=pose\_resolved} y
\texttt{actual\_room} resuelto desde la pose final. \textbf{Fase 3 cerrada.}

La figura~\ref{fig:resolucion-room-fase3} resume el mecanismo: el contexto vivo
(latched) y la pose del robot se combinan en el nodo de resultado para producir
la habitación real, con \emph{fallback} explícito y trazable al eco cuando no
hay contexto.
...
\begin{tikzpicture}[
  ...
  sim/.style={draw, rounded corners, align=center, minimum width=3.0cm,
              minimum height=0.85cm, fill=blue!8},
  ros/.style={draw, rounded corners, align=center, minimum width=3.0cm,
              minimum height=0.85cm, fill=green!8},
  decision/.style={draw, diamond, aspect=2.4, align=center, fill=orange!12,
                   inner sep=1pt},
  outnode/.style={draw, rounded corners, align=center, minimum width=3.0cm,
              minimum height=0.85cm, fill=green!18},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
  \node[sim] (ctxpub) {tfg-sim:\\\texttt{publish\_room\_context\_live}};
  \node[ros, right=1.6cm of ctxpub] (ctxtopic) {\texttt{/vlmap/room\_context}\\(latched, JSON)};
  \node[ros, below=of ctxtopic] (odom) {\texttt{/odom}\\pose en frame map};
  \node[ros, below=of odom] (result) {\texttt{/move\_base/result}\\status = 3};
  \node[decision, right=1.4cm of odom] (has) {\,¿contexto\\+ pose?\,};
  \node[outnode, right=1.4cm of has, yshift=0.9cm] (resolved) {\texttt{room\_at\_pose}\\\texttt{room\_source=}\\\texttt{pose\_resolved}};
  \node[ros, right=1.4cm of has, yshift=-0.9cm, fill=orange!10] (echo) {eco de la pedida\\\texttt{room\_source=}\\\texttt{requested\_echo}};
  \node[outnode, below=1.0cm of echo] (navres) {\texttt{/vlmap/navigation\_result}\\token + success + actual\_room};

  \draw[arrow] (ctxpub) -- (ctxtopic);
  \draw[arrow] (ctxtopic) -- (has);
  \draw[arrow] (odom) -- (has);
  \draw[arrow] (result) -| (has);
  \draw[arrow] (has) -- node[above,sloped]{sí} (resolved);
  \draw[arrow] (has) -- node[below,sloped]{no} (echo);
  \draw[arrow] (resolved) |- (navres);
  \draw[arrow] (echo) -- (navres);
\end{tikzpicture}%
}
\caption{Resolución de \texttt{actual\_room} en Fase 3. El contexto semántico
vivo (latched) y la pose real del robot (\texttt{/odom}, frame \texttt{map}) se
combinan en el nodo de resultado: si ambos están presentes,
\texttt{room\_at\_pose} devuelve la habitación por centroide más cercano
(\texttt{pose\_resolved}); si no, se mantiene el eco trazable de la habitación
pedida (\texttt{requested\_echo}).}
\label{fig:resolucion-room-fase3}
\end{figure}

\paragraph{Giro más lento (anti-convulsión).} En las pruebas el HSR aún
``convulsionaba'' levemente al girar con \texttt{max\_vel\_theta}~$=0{,}22$.
Se reduce de nuevo, sin volver al \texttt{0{,}12} que resultó demasiado lento:
\texttt{max\_vel\_theta}~$=0{,}16$, \texttt{acc\_lim\_theta}~$=0{,}20$,
\texttt{min\_in\_place\_vel\_theta}~$=0{,}06$, y en el suavizador
\texttt{cmd\_vel\_max\_angular}~$=0{,}16$ con
\texttt{cmd\_vel\_max\_angular\_accel}~$=0{,}10$ (la rampa angular más baja es
la que de verdad quita el tirón).

\subsection*{28 de mayo de 2026 (tercera iteración) --- Fase 4: percepción en Habitat (Opción A)}

Decidida la arquitectura de percepción: \textbf{Opción A}. ROS navega y los
sensores físicos del HSR no se usan para verificar; la verificación visual la
sigue haciendo Habitat, renderizando la pose final donde sí están los objetos
ya colocados (botella, taza, baloncesto, peluche, portátil). Así no se porta
percepción a ROS y se reutiliza todo el trabajo de colocación. La alternativa
(mundo espejo en Gazebo con los objetos reconstruidos en SDF/URDF) se descarta
por coste y por romper la regla arquitectónica.

\begin{enumerate}
  \item \textbf{La pose final viaja en el resultado}: el nodo de ROS guarda la
        pose del robot como \texttt{(x, y, yaw)} (yaw extraído del cuaternión) y
        \texttt{build\_navigation\_result\_payload} la registra en
        \texttt{metadata.final\_pose}. Así \texttt{tfg-sim} recibe la pose sin
        una segunda suscripción.
  \item \textbf{Transformación de frames}: nueva clase \texttt{Rigid2D}
        (\texttt{tx, ty, theta, scale}) que expresa \texttt{T\_habitat\_from\_map}.
        Para el Gazebo vacío actual ambos frames coinciden, así que la identidad
        es el valor por defecto; sólo hará falta una transformación no trivial
        cuando Gazebo cargue un mapa con origen/orientación distintos.
  \item \textbf{\texttt{PoseSyncBridge}}: dado un \texttt{navigation\_result},
        extrae \texttt{final\_pose}, la transforma al frame de Habitat y llama a
        un \emph{verifier} inyectado (\texttt{verifier(pose, result)}). En
        producción ese callback teletransporta el agente de Habitat a esa pose y
        ejecuta YOLOE; en los tests es un \emph{fake}. La lógica de transporte y
        transformación es pura y sin dependencia de ROS ni Habitat.
  \item \textbf{Tests}: nuevo \texttt{tests/test\_pose\_sync.py} (identidad,
        traslación, rotación, inverso \emph{round-trip}, extracción de pose y
        \emph{hook} del verifier). Total \textbf{38 pruebas en verde}.
\end{enumerate}

La figura~\ref{fig:bucle-percepcion-fase4} muestra el bucle completo previsto
para la demo (Fase 5): la decisión semántica y la verificación viven en
\texttt{tfg-sim}; ROS sólo navega y devuelve la pose real, que el
\texttt{PoseSyncBridge} reproyecta a Habitat para que YOLOE confirme.
...
\begin{tikzpicture}[
  ...
  ros/.style={draw, rounded corners, align=center, minimum width=3.1cm,
              minimum height=0.85cm, fill=green!8},
  decision/.style={draw, diamond, aspect=2.6, align=center, fill=orange!12,
                   inner sep=1pt},
  done/.style={draw, rounded corners, align=center, minimum width=2.6cm,
               minimum height=0.85cm, fill=green!18},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
  \node[sim] (query) {tfg-sim: query\\``find laptop''};
  \node[sim, below=of query] (decide) {decide habitación\\+ candidato (VLMaps)};
  \node[ros, right=1.8cm of decide] (nav) {tfg-ros: \texttt{move\_base}\\navega el HSR};
  \node[ros, below=of nav] (res) {\texttt{navigation\_result}\\+ \texttt{final\_pose}};
  \node[sim, left=1.8cm of res] (sync) {\texttt{PoseSyncBridge}\\map$\rightarrow$Habitat};
  \node[sim, below=of sync] (yoloe) {teleport Habitat\\+ YOLOE};
  \node[decision, right=1.6cm of yoloe] (ok) {¿confirmado?};
  \node[done, right=1.4cm of ok] (found) {objeto\\encontrado};

  \draw[arrow] (query) -- (decide);
  \draw[arrow] (decide) -- node[above]{SemanticGoal} (nav);
  \draw[arrow] (nav) -- (res);
  \draw[arrow] (res) -- (sync);
  \draw[arrow] (sync) -- (yoloe);
  \draw[arrow] (yoloe) -- (ok);
  \draw[arrow] (ok) -- node[above]{sí} (found);
  \draw[arrow] (ok.south) |- node[pos=0.25,left]{no: otro candidato} (decide.south);
\end{tikzpicture}%
}
\caption{Bucle de la demo end-to-end (Fase 5) bajo la Opción A. \texttt{tfg-sim}
decide y verifica; \texttt{tfg-ros} sólo navega y devuelve la pose real. El
\texttt{PoseSyncBridge} reproyecta esa pose al frame de Habitat, donde YOLOE
confirma sobre los objetos colocados. Si no se confirma, la política elige otro
candidato y se repite el ciclo. ROS nunca percibe.}
\label{fig:bucle-percepcion-fase4}
\end{figure}

\subsection*{7 de junio de 2026: validación semántica multiclase del VLMap ROS/Gazebo}

Se ha cerrado la validación del mapa construido desde una captura RGB-D del HSR
en Gazebo. El resultado principal es que el fichero \texttt{vlmaps.h5df}
generado por el flujo ROS/Gazebo tiene el mismo formato operativo que los mapas
HSSD que ya funcionaban en \texttt{tfg-sim}; el problema observado no estaba en
la construcción del mapa, sino en la forma de medirlo.

El diagnóstico inicial usaba el score crudo de una sola categoría. Ese criterio
no es válido para evaluar VLMaps porque las puntuaciones CLIP de categorías
aisladas son muy colineales: dos mapas de una única clase pueden correlacionar
casi igual aunque describan zonas distintas. La validación nueva replica el uso
nativo de VLMaps: se inicializa el vocabulario completo con
\texttt{init\_categories(mp3dcat)}, se añaden las categorías solicitadas que no
están en MP3D y cada vóxel queda asignado sólo a la categoría que gana el
\texttt{argmax} frente al resto del vocabulario y la clase de fondo
\texttt{other}.

Además de esta validación, se dejan documentados los arreglos que hicieron
posible el flujo completo: captura robusta ante fallos TF con fallback a
\texttt{/odom}, carga correcta del directorio del run en \texttt{VLMap.load\_map},
\texttt{PYTHONPATH} adecuado en el menú, uso de \texttt{++} en claves Hydra ya
existentes, inyección de \texttt{params.gs} y \texttt{params.cs} al cargar la
configuración aislada y eliminación de la inicialización vacía de categorías.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
Mapa evaluado & Celdas ocupadas & IoU medio & IoU máximo & Par máximo \\
\midrule
small\_house ROS/Gazebo & 25\,629 & 0.00010 & 0.00126 & refrigerator-sink \\
HSSD 102344280\_0 & 21\,453 & 0.0120 & 0.1655 & table-chair \\
\bottomrule
\end{tabular}
\caption{Resultado del índice semántico multiclase para categorías de
habitaciones distintas. En el mapa capturado desde ROS/Gazebo las máscaras son
prácticamente disjuntas; en HSSD el mayor solape aparece entre mesa y silla, que
es una proximidad semánticamente esperable.}
\end{table}
...
\caption{Mapa de categoría dominante por celda para la captura
\texttt{small\_house} (izquierda) y para HSSD \texttt{102344280\_0} (derecha).
La captura ROS/Gazebo separa cama, cocina, zona común y ausencia de baño con un
solape medio muy bajo.}
\end{figure}
