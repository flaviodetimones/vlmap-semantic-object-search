...
TARGET_QUERIES = 500
PRIMARY_HIGHER_BETTER = {"hit@1", "mass_in_expected_ratio"}
PRIMARY_LOWER_BETTER = {"wrong_room_mass_ratio"}
SECONDARY_HIGHER_BETTER = {"hit@5", "iou_topmass50"}
SECONDARY_LOWER_BETTER = {"n_components"}
HIGHER_BETTER = PRIMARY_HIGHER_BETTER | SECONDARY_HIGHER_BETTER
LOWER_BETTER = PRIMARY_LOWER_BETTER | SECONDARY_LOWER_BETTER
...
def _choose_winner(global_rows: List[dict], retained_metrics: List[str]) -> str:
    ...
    primary_metrics = [
        ...
        if metric in (PRIMARY_HIGHER_BETTER | PRIMARY_LOWER_BETTER)
    ]
...
def _write_phase_summary(
    *,
    ...
    lines = [
        ...
        f"Winner: **{winner}**",
        "",
        f"Primary metrics: {', '.join(m for m in retained_metrics if m in (PRIMARY_HIGHER_BETTER | PRIMARY_LOWER_BETTER)) or 'none'}",
        f"Secondary metrics: {', '.join(m for m in retained_metrics if m in (SECONDARY_HIGHER_BETTER | SECONDARY_LOWER_BETTER)) or 'none'}",
...
def main() -> None:
    ...
    config = {
        ...
        "winner": winner,
        "primary_metrics": {
            "higher_better": sorted(PRIMARY_HIGHER_BETTER),
            "lower_better": sorted(PRIMARY_LOWER_BETTER),
        },
        "secondary_metrics": {
            "higher_better": sorted(SECONDARY_HIGHER_BETTER),
            "lower_better": sorted(SECONDARY_LOWER_BETTER),
        },
        "removed_metrics": removed_metrics,
