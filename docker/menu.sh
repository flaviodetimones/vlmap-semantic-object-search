            ...
            while true; do
                ...
                case "$sub" in
                    r|R)
                        ...
                        echo ""
                        echo "  ── Step 3 · Pre-label room regions ──────────────────────────"
                        echo "  Build semantic room map from VLMap heatmaps."
                        echo "  Add refine=true to use GPT-4o Vision for label correction."
                        echo ""
                        echo "    python $APP/build_room_map.py data_paths=docker scene_id=0"
                        echo "    python $APP/build_room_map.py data_paths=docker scene_id=0 refine=true"
                        echo ""
                        echo "  ── Step 4 · Interactive LLM navigation (GPT-4o-mini) ────────"
