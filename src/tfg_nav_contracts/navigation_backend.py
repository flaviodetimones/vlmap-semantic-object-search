...
class RosNavigationBackend(NavigationBackend):
    ...
    def _make_pose_stamped(self, x: float, y: float, yaw: float = 0.0) -> Dict[str, Any]:
    ...
    def _goal_yaw(self, goal: SemanticGoal) -> float:
        metadata = goal.metadata or {}
        raw_yaw = metadata.get("yaw", metadata.get("theta", 0.0))
        try:
    ...
    def _semantic_goal_payload(self, goal: SemanticGoal, token: str) -> Dict[str, Any]:
        ...
        x, y = goal.map_pose
        yaw = self._goal_yaw(goal)
        return {
            "type": goal.type.value,
            "map_pose": self._make_pose_stamped(x, y, yaw),
    ...
    def _nav_goal_payload(self, goal: SemanticGoal) -> Dict[str, Any]:
        x, y = goal.map_pose
        return self._make_pose_stamped(x, y, self._goal_yaw(goal))
...
class RoomContextPublisher:
    """Publishes live semantic room context from tfg-sim to tfg-ros.

    Mirrors :class:`RosNavigationBackend` (roslibpy over rosbridge, no hard
    ``rospy`` dependency) but in the opposite direction: tfg-sim is the source
    of truth for the room layout and pushes it to ROS as a latched JSON topic.

    The payload schema matches ``vlmap_semantic_server.dynamic_context``:
        {
          "rooms":   [{"room_id": str, "x": float, "y": float,
                        "aliases": [str], "metadata": {...}}, ...],
          "priors":  {"<category>": [["<room_id>", score], ...]},
          "maps":    {"<key>": "/shared/heatmaps/<scene>/<cat>.png"}  # optional
        }

    Maps/heatmaps travel as *references* into the shared bind mount, never
    embedded, so rosbridge is not saturated.
    """
    ...
    def __init__(
        ...
        *,
        host: str = "tfg-ros",
        port: int = 9090,
        context_topic: str = "/vlmap/room_context",
        context_topic_type: str = "std_msgs/String",
        auto_connect: bool = True,
        publish_settle_s: float = 0.1,
        client: Any = None,
        topic_factory: Optional[Callable[[Any, str, str], Any]] = None,
        ...
        self.context_topic_type = context_topic_type
        self.publish_settle_s = float(publish_settle_s)
        self._sleep = sleep_fn or time.sleep

        if client is None or topic_factory is None or message_factory is None:
            roslibpy = RosNavigationBackend._import_roslibpy()
            self._client = client or roslibpy.Ros(host=self.host, port=self.port)
            self._topic_factory = topic_factory or (
                lambda ros_client, name, type_name: roslibpy.Topic(
                    ros_client, name, type_name, latch=True, queue_size=1
                )
            )
            self._message_factory = message_factory or roslibpy.Message
        else:
    ...
    def _bind_topics(self) -> None:
        if self._context_pub is not None:
            return
        self._context_pub = self._topic_factory(
            self._client, self.context_topic, self.context_topic_type
        )
    ...
    def build_payload(
        ...
        *,
        ...
        """Assemble the JSON-able context dict from room dicts.

        ``rooms`` items must carry ``room_id`` (or ``label``) plus a pose
        (``x``/``y`` or ``row``/``col``). ``maps`` values are paths under
        ``/shared/`` — they are passed through untouched.
        """
        payload: Dict[str, Any] = {"rooms": list(rooms)}
        if priors:
            payload["priors"] = dict(priors)
        if maps:
            payload["maps"] = {str(k): str(v) for k, v in maps.items()}
        return payload
    ...
    def publish(
        ...
        *,
        ...
        """Publish the room context as a latched JSON String. Returns the JSON."""
        self._ensure_connected()
        self._bind_topics()
        payload = self.build_payload(rooms, priors=priors, maps=maps)
        data = json.dumps(payload, ensure_ascii=False)
        self._context_pub.publish(self._message_factory({"data": data}))
        if self.publish_settle_s > 0.0:
            self._sleep(self.publish_settle_s)
        return data
