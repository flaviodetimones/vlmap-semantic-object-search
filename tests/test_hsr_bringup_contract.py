...
def test_hsr_launch_wires_cmd_vel_smoother_without_changing_hsr_topic_contract():
    launch_path = BRINGUP / "launch" / "hsr_gazebo_move_base.launch"
    tree = ET.parse(launch_path)
    root = tree.getroot()

    assert root.tag == "launch"
    args = {arg.attrib.get("name"): arg.attrib.get("default") for arg in root.iter("arg")}
    ...
    smoother = [
        ...
        if node.attrib.get("type") == "hsr_cmd_vel_smoother.py"
    ]
    assert len(smoother) == 1
    move_base = [
        node
        for node in root.iter("node")
        if node.attrib.get("pkg") == "move_base" and node.attrib.get("type") == "move_base"
    ]
    assert len(move_base) == 1
    remaps = {(r.attrib.get("from"), r.attrib.get("to")) for r in move_base[0].iter("remap")}
