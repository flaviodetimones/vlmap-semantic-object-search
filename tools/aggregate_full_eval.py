...
ADDITIONAL_METRICS = [
    "room_sr",
    "inspections_before_success",
    "final_room_accuracy",
    "preview_no_action_rate",
    "path_stopped_early_rate",
    "mean_executor_actions",
    "mean_room_transitions",
]
...
def _build_compare_rows(rows_by_method: Dict[str, List[dict]]) -> List[dict]:
    ...
    ordered_keys = [key for key in EXPECTED_KEYS if key in rows_by_method]
    compare_rows: List[dict] = []
    for idx, qid in enumerate(all_ids, 1):
        row0 = next((per_method[k][qid] for k in ordered_keys if qid in per_method[k]), {})
        record = {
            "idx": idx,
            "id": qid,
...
def _build_aggregate_rows(rows_by_method: Dict[str, List[dict]]) -> List[dict]:
    ...
    for key in [k for k in EXPECTED_KEYS if k in rows_by_method]:
...
def build_cross_scene_rows(scene_global_rows: Dict[str, List[dict]]) -> List[dict]:
    rows: List[dict] = []
    for method, per_scene_rows in scene_global_rows.items():
        row = {"method": method, "n_scenes": len(per_scene_rows)}
        for metric in MAIN_METRICS + ADDITIONAL_METRICS:
            vals = [r.get(metric, float("nan")) for r in per_scene_rows]
            row[f"{metric}_mean"] = _safe_mean(vals)
            row[f"{metric}_std"] = _safe_std(vals)
        rows.append(row)
    rows.sort(key=lambda r: EXPECTED_KEYS.index(r["method"]) if r["method"] in EXPECTED_KEYS else 999)
    return rows
...
def _pair_delta_rows(global_rows: Dict[str, dict]) -> List[dict]:
    ...
    for label, lhs, rhs in pairs:
        if lhs not in global_rows or rhs not in global_rows:
            continue
        row = {"contrast": label, "lhs": lhs, "rhs": rhs}
        for metric in MAIN_METRICS + ADDITIONAL_METRICS:
...
def _write_aggregate_md(rows: List[dict], out_path: Path) -> None:
    ...
    global_by_method = {r["method"]: r for r in global_rows}
    ...
    for row in global_rows:
        lines.append(
            ...
            f"{_fmt(row['cfr'])} | {_fmt(row['ct2r'])} | {_fmt(row['rooms_before_success'])} | "
            f"{_fmt(row['wrong_visits'])} | "
    ...
    lines += [
        ...
        "## Additional metrics",
        "",
        "| method | Room SR | Inspections Before Success | Final Room Acc. | Preview No Action | Path Stopped Early | Mean Executor Actions | Mean Room Transitions |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in global_rows:
        lines.append(
            f"| {row['method']} | {_fmt(row['room_sr'])} | {_fmt(row['inspections_before_success'])} | "
    ...
    if delta_rows:
        lines += [
            ...
            "## Contrasts",
            "",
            "| contrast | ΔSR | ΔObject SR | ΔCFR | ΔCT2R | ΔRooms Before Success | ΔWrong Visits | ΔMean Pose Updates | ΔEarly Stop Rate |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
        for row in delta_rows:
            lines.append(
                f"| {row['contrast']} ({row['lhs']} − {row['rhs']}) | "
                f"{_fmt(row['sr'])} | {_fmt(row['object_sr'])} | {_fmt(row['cfr'])} | {_fmt(row['ct2r'])} | "
                f"{_fmt(row['rooms_before_success'])} | {_fmt(row['wrong_visits'])} | "
                f"{_fmt(row['mean_pose_updates'])} | {_fmt(row['early_stop_rate'])} |"
            )
...
def _write_cross_scene_md(rows: List[dict], out_path: Path) -> None:
    ...
    for row in rows:
        lines.append(
            f"| {row['method']} | "
            f"{_fmt(row['sr_mean'])} ± {_fmt(row['sr_std'])} | "
            f"{_fmt(row['object_sr_mean'])} ± {_fmt(row['object_sr_std'])} | "
            f"{_fmt(row['cfr_mean'])} ± {_fmt(row['cfr_std'])} | "
            f"{_fmt(row['ct2r_mean'])} ± {_fmt(row['ct2r_std'])} | "
            f"{_fmt(row['rooms_before_success_mean'])} ± {_fmt(row['rooms_before_success_std'])} | "
            f"{_fmt(row['wrong_visits_mean'])} ± {_fmt(row['wrong_visits_std'])} | "
            f"{_fmt(row['mean_pose_updates_mean'])} ± {_fmt(row['mean_pose_updates_std'])} | "
            f"{_fmt(row['early_stop_rate_mean'])} ± {_fmt(row['early_stop_rate_std'])} |"
        )
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def aggregate_scene(manifests: Dict[str, Path]) -> tuple[list[dict], list[dict]]:
    ...
    return compare_rows, aggregate_rows
...
def main() -> None:
    ...
    parser.add_argument("--cross-scene-csv", type=Path, default=None)
    parser.add_argument("--cross-scene-md", type=Path, default=None)
    ...
    compare_rows, aggregate_rows = aggregate_scene(manifests)

    _write_csv(compare_rows, args.out_csv)
    _write_compare_md(compare_rows, args.out_md, [k for k in EXPECTED_KEYS if k in manifests])
    _write_csv(aggregate_rows, args.aggregate_csv)
    _write_aggregate_md(aggregate_rows, args.aggregate_md)

    if args.cross_scene_csv or args.cross_scene_md:
        global_rows = [r for r in aggregate_rows if r["slice_kind"] == "global"]
        cross_rows = build_cross_scene_rows({
            row["method"]: [row] for row in global_rows
        })
        if args.cross_scene_csv:
            _write_csv(cross_rows, args.cross_scene_csv)
        if args.cross_scene_md:
            _write_cross_scene_md(cross_rows, args.cross_scene_md)
