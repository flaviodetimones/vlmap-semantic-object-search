...
\textbf{Estado actual}: \textcolor{partialcol}{tooling listo, comparativa
sistemática pendiente}. El repositorio ya incluye un \emph{runner} batch
(\texttt{tools/run\_hssd\_nav\_compare.sh}) que ejecuta el mismo lote de
\emph{queries} contra el baseline y el executor, y un resumidor
(\texttt{tools/compare\_nav\_runs.py}) que genera CSV + markdown con la
comparación query-a-query. El trabajo restante se centra en usar esa
infraestructura sobre escenas representativas, recoger métricas y documentar
las diferencias cualitativas de heatmap y trayectoria.
