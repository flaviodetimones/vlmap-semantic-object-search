...
\subsubsection*{Capa room-aware ya disponible (Fases A, B, C)}
...
\midrule
Fase D: selector de habitación &
  \textcolor{pendingcol}{\textbf{Núcleo pendiente}} \\
Fase E: candidatos intra-habitación & \textcolor{pendingcol}{Pendiente} \\
Fase F: acciones de alto nivel & \textcolor{pendingcol}{Pendiente} \\
...
objeto) y de toda la infraestructura para usarlos. Lo que falta es la
\textbf{política} que los integre en una decisión coherente: actualmente el
robot conoce los priors de habitación, sabe qué mueble buscar y sabe
verificar con YOLOE, pero la selección final del candidato sigue dominada por
el ranking global del heatmap, no por la jerarquía habitación~$\rightarrow$
mueble~$\rightarrow$ objeto.

El cierre de esa política es el bloque de fases D--H descrito en la
Sección~\ref{sec:room-aware}. No es una tarea de infraestructura: es la pieza
que convierte un sistema con muchos componentes semánticos disponibles en un
buscador jerárquico real, y es exactamente la pieza que permite demostrar la
hipótesis central del TFG.
...
D & Selector de habitación & B + C & 1--2 sesiones \\
E & Candidatos intra-habitación & D + pipeline M2 & 1 sesión \\
F & Acciones de alto nivel & E & 1--2 sesiones \\
G & LLM estratégico con estado & B + C + F & 2 sesiones \\
H & Métricas de evaluación & F & 1 sesión \\
\midrule
  & \textbf{Esfuerzo restante estimado} & & \textbf{6--8 sesiones} \\
\bottomrule
\end{tabular}
\end{center}
