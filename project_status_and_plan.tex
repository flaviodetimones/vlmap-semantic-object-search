...
objeto) y de toda la infraestructura para usarlos. La primera decisión
jerárquica real ya existe: el selector de habitación (Fase~D) decide ahora
qué habitación merece inspección antes de elegir el candidato concreto.

Lo que falta es cerrar el resto de la política room-aware: restringir de forma
más explícita la búsqueda al subconjunto intra-habitación (Fase~E),
formalizarla como acciones de alto nivel (Fase~F), permitir razonamiento
estratégico con estado estructurado (Fase~G) y medirla con métricas propias
(Fase~H). Ese bloque restante es el que convertirá el sistema actual en un
buscador jerárquico completo y defendible experimentalmente.
